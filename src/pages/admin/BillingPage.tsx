...
type Lesson = { id: string; student_name: string; guardian_name: string | null; subject: string | null; start_at: string; duration_minutes: number; price: number; payment_status: string; teacher: string };

type Filter = "all" | "thiago" | "mayara";
...
export default function BillingPage() {
  ...
  const filtered = useMemo(
    () => filter === "all" ? lessons : lessons.filter(l => l.teacher === filter),
    [lessons, filter]
  );

  const grandTotal = useMemo(
    () => lessons.reduce((s, l) => s + Number(l.price), 0),
    [lessons]
  );

  const grouped = useMemo(() => {
    const pending = filtered.filter(l => l.payment_status === "pendente");
  ...
  const totals = useMemo(() => {
    const pend = filtered.filter(l => l.payment_status === "pendente").reduce((s, l) => s + Number(l.price), 0);
    const paid = filtered.filter(l => l.payment_status === "pago").reduce((s, l) => s + Number(l.price), 0);
    return { pend, paid };
  }, [filtered]);
  ...
  const filterButtons: { key: Filter; label: string }[] = [
    { key: "all", label: "Visão Geral (Ambos)" },
    { key: "thiago", label: "Apenas Thiago" },
    { key: "mayara", label: "Apenas Mayara" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-5 bg-gradient-to-br from-primary/10 to-accent/10 border-primary/20">
        <div className="text-xs text-muted-foreground uppercase">Total família (Pago + Pendente, ambos professores)</div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <Card className="p-5"><div className="text-xs text-muted-foreground uppercase">Recebido</div><div className="text-3xl font-bold text-success mt-1">{fmt(totals.paid)}</div></Card>
      </div>
                ...
                {g.lessons.map(l => (
                  <div key={l.id} className="flex items-center justify-between gap-3 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-muted-foreground">· {format(new Date(l.start_at), "dd/MM HH:mm", { locale: ptBR })}</span>
                    </div>
