...
export default function BlocksPage() {
  ...
  const [recForm, setRecForm] = useState({ title: "Escola", weekday: 1, start_time: "07:00", end_time: "13:00" });
  const [oneForm, setOneForm] = useState({ title: "Lazer", start_at: "", end_at: "" });
  ...
  const addRecurring = async () => {
    const { error } = await supabase.from("blocks").insert({ title: recForm.title, block_type: "recurring", weekday: recForm.weekday, start_time: recForm.start_time, end_time: recForm.end_time });
  ...
  const addOneOff = async () => {
    ...
    const { error } = await supabase.from("blocks").insert({ title: oneForm.title, block_type: "one_off", start_at: new Date(oneForm.start_at).toISOString(), end_at: new Date(oneForm.end_at).toISOString() });
